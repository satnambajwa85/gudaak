	...
	<?php
		...
		foreach($listCar as $subCat){
				...
				foreach($subCats1S as $subCat){
					if(!in_array($subCat->stream_id,$listArr))
						{
							$listArr[]=$subCat->stream_id;
					?>
					<div class="col-md-4 pd0 fl">
					<div class="col-md-12 pdleft career-lib">
	<?php 
			$filename = ''.$subCat->stream->image.'';
			$path=Yii::getPathOfAlias('webroot.uploads.stream.small') . '/';
			$file=$path.$filename;
			if (file_exists($file)){ ?>
			<?php echo CHtml::link('<img src="'.Yii::app()->baseUrl.'/uploads/stream/small/'.$subCat->stream->image.'"/>',array('user/stream','id'=>''.$subCat->stream_id.''),array('class'=>'fl'));?>
	<?php 	}else{ ?>
			<?php echo CHtml::link('<img src="'.Yii::app()->baseUrl.'/uploads/stream/small/noimage.jpg"/>',array('user/stream','id'=>''.$subCat->stream_id.''),array('class'=>'fl'));?>
		
	<?php } ?>
	<div class="clear"></div>
	<?php echo CHtml::link('<h1>'.substr($subCat->stream->name,0,20).'..</h1>',array('user/stream','id'=>''.$subCat->stream_id.''),array('title'=>$subCat->stream->name));?>
	<p><?php echo substr($subCat->stream->description,0,70);?></p>
	<div class="col-md-12 career-hot-links">
	<?php echo CHtml::link('Read more..',array('user/readFull','id'=>''.$subCat->stream_id.''),array('class'=>'pull-left','title'=>'Read Full.'));?>
