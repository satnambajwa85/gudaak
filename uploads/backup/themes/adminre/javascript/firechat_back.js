...
(function(Firebase) {
  ...
  Firechat.prototype = {
    ...
    _loadUserMetadata: function(onComplete) {
      ...
      this._userRef.transaction(function(current) {
        if (!current || !current.id || !current.name || !current.type ) {
          return {
            id: self._userId,
            name: self._userName,
            type:self._userType
  ...
  /*P:edited*/
  Firechat.prototype.setUser = function(userId, userName,userType,callback) {
    var self = this;
    ...
    self._firebase.root().child('.info/authenticated').on('value', function(snapshot) {
      if (snapshot.val() === true) {
        ...
        /*P:edited*/
        self._userType = userType.toString();
    ...
    query.once('value', function(snapshot) {
      ...
      for (var userNameKey in usernames) {
        var sessions = usernames[userNameKey],
            userName, userId,userType,usernameClean;
        ...
        for (var sessionId in sessions) {
          ...
          userType=sessions[sessionId].type;
