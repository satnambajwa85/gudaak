...
function menuscroll(el)
{
	if(window.localStorage)
	{
		//var toppos=$(el).offset().top;
		var pos=window.location.href.substr(window.location.href.lastIndexOf('=')+1);
		//var toppos=$("#nameT-"+pos).offset().top;
		var toppos=$("#nav").offset().top;
		if(toppos<0)
			toppos=0-toppos;
			
		//toppos=toppos+$(el).offset().top;
		//var addval=$("#scrollbox6").height()-$("#scrollbox6")[0].scrollHeight;
		//toppos=toppos+addval;
		window.localStorage.setItem("toppos",toppos);
	}
}
...
(function($){
				...
				/*----Added for scroll position on menu----*/
				//console.log($('li.active').last().html());
				var menu=$('li.active').last();
				var toppos=$('li.active').last().offset().top;
				//console.log("position is "+toppos+" : "+$('#scrollbox6').offset().top+" : "+$(menu).html());
				$('#scrollbox6').scrollTop(toppos);
				console.log(window.localStorage.getItem("toppos") +" scroll box 6 scroll height is "+$("#scrollbox6").scrollHeight);
				
				if(window.localStorage.getItem("toppos")!=0)
				{
					console.log('local storage has got some value '+window.localStorage.getItem("toppos"));
					toppos=window.localStorage.getItem("toppos");
					$('#scrollbox6').scrollTop(toppos);
					window.localStorage.setItem("toppos",0);
				}
				/*----Added for scroll position on menu----*/
