...
class Cities extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('states_id', 'required'),
			array('states_id, status', 'numerical', 'integerOnly'=>true),
			array('name, code, others', 'length', 'max'=>45),
			array('description', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, states_id, name, description, status, code, others', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'clientProfiles' => array(self::HAS_MANY, 'ClientProfiles', 'cities_id'),
			'suppliers' => array(self::HAS_MANY, 'Suppliers', 'cities_id'),
		);
	}
