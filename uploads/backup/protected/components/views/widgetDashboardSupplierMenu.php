<?php //echo "printing";CVarDumper::dump($this->projectStatus,10,1);die; 
	...
	$awarded="<ul class='submenu collapse ".((isset($_REQUEST['stat']) && $_REQUEST['stat']==0)?'in':'')."' id='projectnew' data-status='0'>";

	$clarification="<ul class='submenu collapse ".(( isset($_REQUEST['stat']) && $_REQUEST['stat']==1)?'in':'')."' id='projectclarification' data-status='1'>";
	$pitched="<ul class='submenu collapse ".(( isset($_REQUEST['stat']) && $_REQUEST['stat']==2)?'in':'')."' id='projectpitched' data-status='2'>";
	$accepted="<ul class='submenu collapse ".(( isset($_REQUEST['stat']) && $_REQUEST['stat']==3)?'in':'')."' id='projectaccepted' data-status='2'>";//Change id below if accepted or started, one will not used
	$started="<ul class='submenu collapse ".(( isset($_REQUEST['stat']) && $_REQUEST['stat']==4)?'in':'')."' id='projectstarted' data-status='4'>";
	$completed="<ul class='submenu collapse ".(( isset($_REQUEST['stat']) && $_REQUEST['stat']==5)?'in':'')."' id='projectcompleted' data-status='5'>";
	$rejected="<ul class='submenu collapse ".(( isset($_REQUEST['stat']) && $_REQUEST['stat']==6)?'in':'')."' id='projectrejected' data-status='6'>";
                ...
                <ul id="nav" class="topmenu">
                    ...
                    <li class="<?php echo (Yii::app()->controller->action->id =='index')?'active':'';?>" >
                        <?php echo CHtml::link( '<span class="figure"><i class="ico-home2"></i></span>
                            <span class="text">Dashboard</span><span class="number"><span class="label label-danger" data-original-title="'.$newProjectCount.' New Leads" data-toggle="tooltip" data-placement="left" >'.$newProjectCount.'</span></span>', array( '/supplier/index&stat'));?>
                    </li>
                   ...
                   <li>
                        ...
                        <ul id="components" class="submenu collapse <?php echo (in_array('profile',array(Yii::app()->controller->action->id, "faq","pastClients"))?'in':'')?> ">
