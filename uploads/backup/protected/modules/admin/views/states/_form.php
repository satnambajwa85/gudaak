...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="row">
		...
		<?php 
			    $list_data	=	array();
				$lists	    =	Countries::model()->findAll();
				foreach($lists as $list)
				{
	         		$list_data[]	=	$list;
				}
				$listData = CHtml::listData($list_data,'id','name');
				echo $form->dropDownList($model,'countries_id',$listData,array('empty'=>'Select a Country'));
		 ?>
	...
	<div class="row">
		...
		<?php 
			    $list_data	=	array();
				$lists	    =	PriceZone::model()->findAll();
				foreach($lists as $list)
				{
	         		$list_data[]	=	$list;
				}
				$listData = CHtml::listData($list_data,'id','title');
				echo $form->dropDownList($model,'price_zone_id',$listData,array('empty'=>'Select a price zone'));
		 ?>
