    ...
    <ul id="nav" class="topmenu">
        ...
        <li class="scoleMain <?php echo (Yii::app()->controller->action->id =='project' && isset($_REQUEST['id']) && $_REQUEST['id']==$project->id)?'active':'';?>" >
            ...
            <ul id="Scope<?php echo $project->id;?>" class="submenu collapse <?php echo (Yii::app()->controller->action->id =='project'  && isset($_REQUEST['id']) && $_REQUEST['id']==$project->id)?'in':'';?>">
                ...
                <?php echo CHtml::ajaxLink('<span class="text">Basic</span>', array('/client/basic','id'=>$project->id),array('success'=>'function(html){
                        ...
                        $("#ProductScope'.$project->id.'").removeClass("activeLink");
                        $("#Budget'.$project->id.'").removeClass("activeLink");
                        $("#Questions'.$project->id.'").removeClass("activeLink");
            }'));?>
                </li>
                <li id="ProductScope<?php echo $project->id;?>" class="<?php echo ($project->other_status >=2)?'active':'';?>">
                    <?php echo CHtml::ajaxLink('<span class="text">Budget</span>', array('/client/basic','id'=>$project->id), array('success'=>'function(html){ $("#content").html(html);$("#basicProject").addClass("hide");$("#productScope").removeClass("hide");$("#Basic'.$project->id.'").parent().parent().addClass("active");$("#ProductScope'.$project->id.'").addClass("activeLink");
                        $("#Basic'.$project->id.'").removeClass("activeLink");
                        $("#Budget'.$project->id.'").removeClass("activeLink");
                        $("#Questions'.$project->id.'").removeClass("activeLink");
                    }'));?>
                </li>
                <li id="Budget<?php echo $project->id;?>" class="<?php echo ($project->other_status >=3)?'active':'';?>">
                    <?php echo CHtml::ajaxLink('<span class="text">Specifications</span>', array('/client/basic','id'=>$project->id), array('success'=>'function(html){ $("#content").html(html);$("#basicProject").addClass("hide");$("#budgetProject").removeClass("hide");$("#Basic'.$project->id.'").parent().parent().addClass("active");$("#Budget'.$project->id.'").addClass("activeLink");
                    $("#Basic'.$project->id.'").removeClass("activeLink");
                    $("#ProductScope'.$project->id.'").removeClass("activeLink");
                    $("#Questions'.$project->id.'").removeClass("activeLink");
                    }'));?>
        ...
        <li>
            ...
            foreach($project->suppliersProjectsProposals as $pr){
                if(in_array($pr->status,array(1,2,3,4)))
                    $cumt = $cumt+1;
            }
            $mesg	=	($cumt>0)?'<span class="arrow"></span>':'';
            echo CHtml::link('<span class="figure"><i class="ico-file6"></i></span><span class="text">Proposals '.(($cumt>0)?'('.$cumt.')':'').'</span>'.$mesg, array('/client/compare','id'=>$project->id), array('success'=>'function(html){ $("#content").html(html);}'));?>
        </li>
        <?php }
        ...
        <li <?php echo ($project->id==$_REQUEST['id'])?'class="active"':'';?>>
            ...
            <ul id="proposal<?php echo $project->id;?>" class="submenu collapse <?php echo ($project->id==$_REQUEST['id'])?'in':'';?>" >
                <?php foreach($project->suppliersProjectsProposals as $company)
                    if(in_array($company->status,array(1,2,3,4))){?>
                <li <?php echo ($company->id==$_REQUEST['id'])?'class="active"':'';?>>
                    ...
                    <ul id="subsubmenu<?php echo $project->id;?>_<?php echo $company->suppliers->id;?>" class="submenu collapse <?php echo ($company->id==$_REQUEST['id'])?'in':'';?>">
                        ...
                        <li id="Portfolio<?php echo $project->id;?>_<?php echo $company->suppliers->id;?>">
                            <?php echo CHtml::ajaxLink('<span class="text">Portfolio</span>', array('/client/portfolio','id'=>$company->suppliers_id), array('success'=>'function(html){ $("#content").html(html);$("#Profile'.$project->id.'_'.$company->suppliers->id.'").removeClass("active");$("#FAQ'.$project->id.'_'.$company->suppliers->id.'").removeClass("active");$("#Portfolio'.$project->id.'_'.$company->suppliers->id.'").addClass("active");}'));?>
                        </li>
                        <li id="FAQ<?php echo $project->id;?>_<?php echo $company->suppliers->id;?>">
                            <?php echo CHtml::ajaxLink('<span class="text">FAQ\'s</span>', array('/client/faq','id'=>$company->suppliers_id), array('success'=>'function(html){ $("#content").html(html);$("#Profile'.$project->id.'_'.$company->suppliers->id.'").removeClass("active");$("#Portfolio'.$project->id.'_'.$company->suppliers->id.'").removeClass("active");$("#FAQ'.$project->id.'_'.$company->suppliers->id.'").addClass("active");}'));?>
                        </li>
                    </ul>
                </li>
                <?php }?>
