...
class CountriesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Countries']))
		{
			...
			if (!empty($_FILES['Countries']['name']['image'])) {
				$tempFile = $_FILES['Countries']['tmp_name']['image'];
				$targetPath	=	$_SERVER['DOCUMENT_ROOT'].$targetFolder;
				$targetFile = $targetPath.'/'.$_FILES['Countries']['name']['image'];
				$pat = $targetFile;
				...
				$img->process('uploads/countries/original/');
				$img->processed;
				...
				if ($img->processed) {
					...
					$img->image_x           = 500;
					$img->file_new_name_body = $newName;
					$img->process('uploads/countries/large/');
					...
					$img->image_x           = 100;
					$img->file_new_name_body = $newName;
					$img->process('uploads/countries/small/');
	...
	public function actionUpdate($id)
	{
		...
		{
			$model->attributes=$_POST['Countries'];
			$targetFolder1 = $_SERVER['DOCUMENT_ROOT'].Yii::app()->baseUrl.'/uploads/countries/';
				...
				$model->image	=	$fileName;
				if($_POST['Countries']['oldImage']!=''){
					@unlink($targetFolder1.'original/'.$_POST['Countries']['oldImage']);
					@unlink($targetFolder1.'large/'.$_POST['Countries']['oldImage']);
					@unlink($targetFolder1.'small/'.$_POST['Countries']['oldImage']);
			 
				}
			}
			else
				$model->image	=	$_POST['Countries']['oldImage'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->id));
		}
