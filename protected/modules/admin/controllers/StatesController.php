...
class StatesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['States']))
		{
			...
			$targetFolder = Yii::app()->request->baseUrl.'/uploads/states/';
			if (!empty($_FILES['States']['name']['image'])) {
				$tempFile = $_FILES['States']['tmp_name']['image'];
				$targetPath	=	$_SERVER['DOCUMENT_ROOT'].$targetFolder;
				$targetFile = $targetPath.'/'.$_FILES['States']['name']['image'];
				$pat = $targetFile;
				...
				$img->process('uploads/states/original/');
				$img->processed;
				...
				if ($img->processed) {
					...
					$img->image_x           = 500;
					$img->file_new_name_body = $newName;
					$img->process('uploads/states/large/');
					...
					$img->image_x           = 100;
					$img->file_new_name_body = $newName;
					$img->process('uploads/states/small/');
	...
	public function actionUpdate($id)
	{
		...
		{
			$model->attributes=$_POST['States'];
			$targetFolder1 = $_SERVER['DOCUMENT_ROOT'].Yii::app()->baseUrl.'/uploads/states/';
				...
				$model->image	=	$fileName;
				if($_POST['States']['oldImage']!=''){
					@unlink($targetFolder1.'original/'.$_POST['States']['oldImage']);
					@unlink($targetFolder1.'large/'.$_POST['States']['oldImage']);
					@unlink($targetFolder1.'small/'.$_POST['States']['oldImage']);
			 
				}
			}
			else
				$model->image	=	$_POST['States']['oldImage'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->id));
		}
