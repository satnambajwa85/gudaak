...
class UserRating extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('rating, user_profiles_id, career_options_id', 'required'),
			array('published, status, user_profiles_id, career_options_id', 'numerical', 'integerOnly'=>true),
			array('rating', 'length', 'max'=>45),
			array('add_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, rating, add_date, published, status, user_profiles_id, career_options_id', 'safe', 'on'=>'search'),
		);
	}
