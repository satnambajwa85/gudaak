...
class UserController extends Controller
{
		...
		return array(
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('index','editProfile','test','tests','detailedReport','collage','liveChat',
							'articlesList','articles','summary','newsUpdates',
							'exploreColleges','shortListedColleges','dynamicCourse','dynamicSearchResult','userShortlistCollage',
							'search','changePassword','application','questionsAnswer','testMail','userProfileUpdate','retakeTest','news','talk','readEvent','summaryDetails',
				),
				'users' => array('@')
	...
	public function actionTalk(){
		$user		=	UserProfiles::model()->findByPk(Yii::app()->user->profileId);
		//echo $user->generateGudaakIds->schools->id;
		$newModel	=	new Tickets;
		if(isset($_POST['Tickets'])){
			$newModel->attributes	=	$_POST['Tickets'];
			$newModel->sender_id	=	Yii::app()->user->profileId;
			$newModel->receiver_id	=	$user->generateGudaakIds->schools_id;
			$newModel->status		=	1;
			$newModel->add_date		=	date('Y-m-d H:i:s');
			if($newModel->save())
		...
		$this->render('talk',array('model'=>$model,'newModel'=>$newModel));	
	}
...
>>>>>>> 1036f2c13eb6c79622b18ad05ae4412c7932baab
}